...
const SignalementArchitecte = () => {
    const router = useRouter();


    const { data: reports, isLoading } = useFetchData('/archimatch_app/admin/architect_reports/', "archireports")
    ...
    const columns = [

        {
            ...
            cell: (props) => {
                const { architect } = props.row.original;
                const firstName = architect.user.first_name;
                const lastName = architect.user.last_name;
                const email = architect.user.email;
                return (
    ...
    return (
        <div>
            ...
            <Card className="mt-5 border ml-2 shadow-none  ">
                ...
                {isLoading ? <TableSkeleton /> : <Table columns={columns} data={reports.data.archireports} />}
            </Card>
...
export default SignalementArchitecte